...
int connectGrid(struct dot start, struct dot end)  //all dots passed to this function are valid, returns whether a box was closed
{
...
int boxClosed(int startIndex, enum direction d)
{

    int i = d, linesJoined = 1, nextIndex = startIndex, boxes = 0;
    while(linesJoined < 4)  //check clockwise
    {
        if(linesJoined == 2)
        {
            diagonal[boxes][1].row = nextIndex/mode;
            diagonal[boxes][1].column = nextIndex%mode;
        }
        i++;
        if(i > 3)
            i = 0;
